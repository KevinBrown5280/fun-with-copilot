...
- **Consensus threshold: 3-of-4** — one persistent dissenter does not veto; 4-of-4 skips directly to polish
...
| **3-of-4** | Near-consensus — dissenter keeps debating with no round cap. Confirmed after 2 consecutive rounds with no vote changes, provided the majority has explicitly rebutted the dissenter's strongest objection. Not valid before round 2. |
| **Stuck** | Synthesizer injects targeted questions each round. At round 6+, surfaces a check-in to you: current tally, core disagreement, continue or stop? |
| **Genuinely irreconcilable** | After 5 consecutive rounds with no vote changes in any split → writes `split-positions.md` automatically with each side's best case and a recommended next step (DEFER / ESCALATE / TEST) |

No hard round cap. No tiebreaker vote. Debates run until the evidence resolves the question or the split is documented for you to decide.
