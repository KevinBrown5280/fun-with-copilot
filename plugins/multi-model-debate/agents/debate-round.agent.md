...
- **Live Research Log check (regular rounds, soft gate):** If any DQ in `context.md` contains terms indicating observable facts (version numbers, GA/Preview/EOS status, pricing), check whether the file contains a `## Live Research Log` section. If the section is absent entirely, log a warning: `"[live-research-missing] <model> round file has no Live Research Log despite fact-dependent DQs — synthesizer will flag training-data-only claims"`. This is a warning, not a hard gate failure — do not retry solely for this reason. The synthesizer's validation (step 1.5) handles downstream treatment.
